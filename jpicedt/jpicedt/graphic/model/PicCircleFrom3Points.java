// PicCircleFrom3Points.java --- -*- coding: iso-8859-1 -*-
...
//
// Version: $Id: PicCircleFrom3Points.java,v 1.36 2013/03/27 07:02:04 vincentb1 Exp $
// Keywords:
...
public class PicCircleFrom3Points extends PicEllipse implements CustomizerFactory {
	...
	// inherited : P_BL, P_BR, P_TR (specification and control points),
	//		specPt_BL, specPt_BR, specPt_TR
	//             P_TL, P_ANGLE_START, P_ANGLE_END (control points only)
	// FIRST_PT left unchanged

	/** index of first ctrl-point */
	public static final int P_1 = PicEllipse.LAST_PT+1 ; // these are our three additional control-points
	...
	protected void updateEllipse(){
		...
		// u and v are the vectors joining P_1 to P_2 and P_3 respectively
		PicVector u = new PicVector(circlePt1,circlePt2);
		PicVector v = new PicVector(circlePt1,circlePt3);
		// m and n are the midpoints of the segments P_1P_2 and P_1P_3
		...
		if (det==0){//flat case
			...
			//First compute center (just a big formula that should do the job)
			PicPoint c= new PicPoint((-v.x*n.x*u.y+v.y*u.x*m.x+v.y*u.y*m.y-v.y*n.y*u.y)/det,
			                         (u.x*v.x*n.x+u.x*v.y*n.y-u.x*m.x*v.x-u.y*m.y*v.x)/det);
			...
			// Set the controling paralleogram (which is a square !)
			...
			if (isCircle){
				...
				// find the correct order between angles...
	...
	protected void updateControlPoints(int numPoint){
		...
		// [SR:todo] => rotate circle's control-points along opposite corner of pts[numPoint].
	...
	public void setCtrlPt(int numPoint, PicPoint pt, EditPointConstraint c){
		if (numPoint == P_CENTER) {
			PicPoint center = getCtrlPt(P_CENTER,null); // save old CENTER
			...
			if(c != null && c instanceof BasicEditPointConstraint)
			{
				...
				bepc = BasicEditPointConstraint.SQUARE;
			super.setCtrlPt(numPoint,pt,bepc); // parallelogram/ellipse control-points, including check for index-out-of-bounds
			if(editConstraint != EditPointConstraint.EditConstraint.SQUARE)
				bepc.setEditConstraint(editConstraint);

			// Reminder : SQUARE constraint ensure parallelogram is a square (hence ellipse is a circle),
			// then "rotate" parallelogram's specification-points along the opposite corner of pts[numPoint]
			updateControlPoints(numPoint); // update control-points P_1,... so that they reflect changes made in the geometry of the superclass
		}
		else {
			switch (numPoint){
			case P_1:
				circlePt1.setCoordinates(pt); break;
			case P_2:
				circlePt2.setCoordinates(pt); break;
			case P_3:
				circlePt3.setCoordinates(pt); break;
			default:
	...
	public PicPoint getCtrlPt(int numPoint,PicPoint src){
		if (numPoint < P_1 ){
			return super.getCtrlPt(numPoint,src);// parallelogram/ellipse control-points, including check for index-out-of-bounds
		}
		else {
			if (src==null) src = new PicPoint();
			switch (numPoint){
			case P_1:
				src.setCoordinates(circlePt1); break;
			case P_2:
				src.setCoordinates(circlePt2); break;
			case P_3:
				src.setCoordinates(circlePt3); break;
			default:
				throw new IndexOutOfBoundsException(new Integer(numPoint).toString());
			}
			return src;
	...
	public CtrlPtSubset getCtrlPtSubset(ConvexZoneGroup csg,BitSet czExtension){
		...
		// Il y a soit un, soit deux point de contrôle dans l'ensemble de zones convexes csg.
		PicPoint inCZPoint1;
		PicPoint inCZPoint2 = null;
		if((bitmap & 1) != 0){
			inCZPoint1 = circlePt1;
			if((bitmap & 2) != 0)
				inCZPoint2 = circlePt2;
			else if((bitmap & 4) != 0)
				inCZPoint2 = circlePt3;
		}
		else if((bitmap & 2) != 0){
	...
	class CtrlPtSubsetCircleFrom3Points implements CtrlPtSubset{
		...
		PicPoint             inCZPoint1;
		PicPoint             inCZPoint2;
		public CtrlPtSubsetCircleFrom3Points(PicCircleFrom3Points cf3p, PicPoint inCZPoint1,
											 PicPoint inCZPoint2){
			this.cf3p = cf3p;
	 ...
	 */
	public ArrayList<PEAction> createActions(ActionDispatcher actionDispatcher, ActionLocalizer localizer,
											 HitInfo hi){
